...
export function DateTimePickerModal({
  ...
  useEffect(() => {
    if (visible && timeScrollRef.current) {
      const y = Math.max(
        0,
        timeIndex * ROW_HEIGHT - (VISIBLE_ROWS * ROW_HEIGHT) / 2 + ROW_HEIGHT / 2
      );
      setTimeout(() => timeScrollRef.current?.scrollTo({ y, animated: false }), 100);
    }
  }, [visible, timeIndex]);
  ...
  return (
    ...
    >
      <View style={styles.body}>
        {!timeOnly && (
            <>
              <Text style={[styles.sectionLabel, { color: hex.textTertiary }]}>Date</Text>
              <ScrollView
                horizontal
              ...
              >
                {DATE_OPTIONS.map((opt, i) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.dateChip,
                      { backgroundColor: i === dateIndex ? ACCENT.primary : hex.surface },
                    ]}
                    activeOpacity={0.8}
                  >
                    <Text
                      style={[
                        ...
                        { color: i === dateIndex ? ACCENT.primaryForeground : hex.text },
                      ]}
                    >
                      {opt.label}
                    </Text>
          ...
          <View
            style={[styles.timeListWrap, { height: VISIBLE_ROWS * ROW_HEIGHT, backgroundColor: hex.surface }]}
          >
            <ScrollView
              ref={timeScrollRef}
              ...
              {TIME_OPTIONS.map((opt, i) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.timeRow,
                    {
                      backgroundColor: i === timeIndex ? `${ACCENT.primary}22` : 'transparent',
